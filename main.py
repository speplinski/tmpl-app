...
import sdl2
import ctypes
from tmpl_app.config import AppConfig
from tmpl_app.core import SDLApp, TextureManager, TransitionManager
from tmpl_app.players import VideoPlayer, ImageSequencePlayer
from tmpl_app.utils import PlaybackStatistics
...
def main():
    parser = argparse.ArgumentParser(description='TMPL App with monitor selection')
    parser.add_argument('--monitor', type=int, default=1,
                      help='Monitor index (0 is usually the main display)')
    args = parser.parse_args()
    
    try:
        app = Application(args.monitor)
        app.run()
